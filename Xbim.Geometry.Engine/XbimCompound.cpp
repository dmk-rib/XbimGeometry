...
// todo: [RIB] remove template specialization hack
template <>
inline Standard_Integer HashCode_Proxy<NCollection_CellFilter<BRepBuilderAPI_VertexInspector>::Cell>(
	const NCollection_CellFilter<BRepBuilderAPI_VertexInspector>::Cell& theKey, 
	const Standard_Integer theUpperBound
	)
{
     return theKey.HashCode(theUpperBound);
}
...
#pragma region Initialisers
		...
		TopoDS_Shape XbimCompound::InitFaces(IEnumerable<IIfcFace^>^ ifcFaces, IIfcRepresentationItem^ theItem, ILogger^ logger)
		{
			...
			for each (IIfcFace ^ ifcFace in ifcFaces)
			{
				...
				{
					System::String^ err = gcnew System::String(failure.GetMessageString());
